...
/****************************************************
 *
 * 　結局CPUがリトル／ビッグエンディアンであるかどうかに関係なく
 * 計算する方法を思いついたので、そちらを使うことにした。
 *
 * 　ちなみにCPUはリトル／ビッグエンディアン以外にも色々な並びがある
 * らしいのですが、ありとあらゆる並びに対応しているはず。ｗ
 *
 */

/****************************************************
 *
 * 　aiffの場合はタグチャンクのサイズに間違った値が入ってる事がある
 * のでは無く、チャンクは必ず2バイトでアラインされてるらしい
 * こちらのソースコード参照させていただきました
 *      http://code.google.com/p/bitspersampleconv2/
 *      http://src.gnu-darwin.org/
 *
 * そしてこんな話もあるので、waveにも付けてあります
